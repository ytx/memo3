...
- 新しいメモとして開きます（既存のタブがある場合は自動で閉じられることがあります）
...
- **タブの切り替え**: Ctrl+Tab で次のタブ、Ctrl+Shift+Tab で前のタブに切り替え（循環）
- **タブを閉じる**: タブの × ボタンをクリック
...
### 空の状態
- タブが1つもない場合、エディタ中央に「新しいメモを作成」ボタンが表示されます
...
### プリセット検索とは
- よく使う検索条件（検索文字列 + タグフィルター）を最大4つまで保存できる機能
- サイドバー下部（ファイルリストの下）に常時表示される4つのプリセットボタン
- ワンクリックで保存した検索条件を即座に適用
- 各ワークスペースで独立したプリセット設定を管理
- プリセットボタンの右側に **×** ボタンがあり、全検索条件を一括クリア可能
...
1. 検索ボックスに検索語を入力（任意）
2. タグフィルターを設定（任意）
3. サイドバー下部の空のプリセットボタン（**+** 表示）をクリック
4. プリセット編集ダイアログが開きます
5. 以下を設定：
   - **タイトル**: 1〜4文字（絵文字も使用可能、例: 📝、TODO、💡、✅）
   - **ボタンの色**: 16色のカラーパレットから選択（タグと同じパレット）
   - **検索条件**: 現在の検索文字列とタグフィルター状態が自動的に設定されます
6. **保存** ボタンをクリック
7. プリセットボタンが設定したタイトルと色で表示されます

**検索条件の割り当て:**
- プリセット編集ダイアログの **現在の検索条件を割り当てる** ボタンをクリックすると、その時点の検索文字列とタグフィルター状態を取得できます
- 既存のプリセットの検索条件を後から更新する際に便利です

### プリセットの適用

1. サイドバー下部のプリセットボタン（**1**、**2**、**3**、**4**）をクリック
2. 保存された検索文字列とタグフィルター状態が即座に適用されます
3. ファイルリストまたは検索結果が更新されます
4. アクティブなプリセットには **太い枠線（3px）** が表示されます

**自動解除:**
- プリセット適用後に、検索ボックスを手動で編集したり、タグフィルターを手動で変更すると、アクティブ状態が自動的に解除されます
- 太い枠線が消え、手動編集モードに戻ります

### プリセットの編集

1. 編集したいプリセットボタンを右クリック
2. プリセット編集ダイアログが開きます
3. 以下を変更可能：
   - **タイトル**: 新しいタイトルを入力（1〜4文字）
   - **ボタンの色**: 別の色を選択
   - **検索条件**: 「現在の検索条件を割り当てる」ボタンで更新
4. **保存** ボタンで変更を保存
5. ダイアログが閉じ、プリセットボタンが更新されます

**注意:**
- 保存ボタンをクリックしても、プリセットは自動適用されません
- 適用するには、更新後のプリセットボタンをクリックしてください

### プリセットの削除

1. 削除したいプリセットボタンを右クリック
2. プリセット編集ダイアログが開きます
3. **削除** ボタン（赤色）をクリック
4. プリセットが削除され、ボタンが空の状態（**+** 表示）に戻ります

### クリアボタン

- プリセットボタンの右側に **×** ボタンがあります
- このボタンをクリックすると、以下がすべてクリアされます：
  - 検索ボックスの内容
  - すべてのタグフィルター状態（すべて「無効」に戻る）
  - アクティブなプリセット状態
- クリア後は、ファイルリスト全体が表示されます
...
- ボタンに **+** 記号が表示されます
- 背景色はサイドバーと同じ（透明）
- クリックすると、現在の検索条件でプリセット作成ダイアログが開きます

**設定済みのプリセット:**
- 設定したタイトル（1〜4文字）が表示されます
- 選択した色で背景が塗られます
- 左クリックで適用、右クリックで編集

**アクティブなプリセット:**
- 太い枠線（3px）が表示されます
- 現在このプリセットの検索条件が適用されていることを示します
...
- 各ワークスペースは独自の4つのプリセット設定を保持
- ワークスペースAで設定したプリセットは、ワークスペースBには影響しません
- ワークスペースを切り替えると、そのワークスペース専用のプリセットボタンが表示されます
- プリセット設定は `sessions.json` にワークスペースごとに保存されます
...
3. 空のプリセットボタンをクリック
4. タイトル「✅」、色「赤」で保存
5. 次回から✅ボタンをクリックするだけで重要なTODOのみ表示
...
3. 空のプリセットボタンをクリック
4. タイトル「🚀」、色「青」で保存
5. 🚀ボタンで進行中プロジェクトファイルを瞬時に絞り込み

**例3: アイデアメモの検索**
1. 検索ボックスに「アイデア」と入力
2. 「完了」タグを「非表示」に設定（完了したものは除外）
3. 空のプリセットボタンをクリック
4. タイトル「💡」、色「黄」で保存
5. 💡ボタンで未完了のアイデアメモを素早く検索
...
**Aboutタブ**
- **GitHub リポジトリ**: プロジェクトのソースコードへのリンク
- **ダウンロードサイト**: 最新版のダウンロードページへのリンク
- **Buy Me A Coffee**: 開発支援へのリンク
...
- **settings**: 設定
- **browser_updated**: バージョン更新通知（赤色）
- **sell**: タグフィルター/タグアイコン
- **draft**: タグなし文書
- **docs**: タグあり文書

すべてのアイコンは現在のテーマに応じて自動的に色が調整されます（browser_updatedは常に赤色）。
...
### タブ操作のショートカット
- **Cmd+N (macOS)**: 新しいメモを作成
- **Ctrl+Tab**: 次のタブに切り替え（最後のタブの次は最初のタブ）
- **Ctrl+Shift+Tab**: 前のタブに切り替え（最初のタブの前は最後のタブ）
...
### 最新版
- **プリセット検索機能**: よく使う検索条件（検索文字列+タグフィルター）を最大4つ保存、サイドバー下部に配置、ワンクリックで適用、ワークスペースごとに独立管理
- **自動バージョンチェック**: 起動時および24時間ごとに新バージョンを自動確認、更新があれば赤いアイコンで通知
- **行間設定**: エディタの行間を1.0〜2.5の範囲で調整可能（デフォルト1.5）
- **設定画面の改善**: エディタ、タグ、Aboutの3タブ構成に変更
  - Aboutタブ追加: GitHub、ダウンロードサイト、Buy Me A Coffeeへのリンク
  - 全般タブ削除: ワークスペース管理はワークスペースセレクターから実施
- **タブキーボードナビゲーション**: Ctrl+Tab で次のタブ、Ctrl+Shift+Tab で前のタブに切り替え（循環式）
