...
⬇️ **[ダウンロード](https://xpenguin.biz/memo3/)**
...
- **空状態UI**: タブが無い時にエディタ中央に「新しいメモを作成」ボタンを表示
...
- **Material Symbols**: UI全体で一貫性のあるアイコンデザイン（folder、search、preview、routine、space_bar、text_decrease、text_increase、settings、browser_updated、sell、draft、docsなど）

### 🔖 プリセット検索機能
- **4つのプリセット**: ワークスペースごとに最大4つの検索条件を保存
- **検索条件保存**: 検索文字列とタグフィルター状態を組み合わせて保存
- **サイドバー下部配置**: ファイルリストの下に常時表示される4つのボタン
- **カスタマイズ**: タイトル（1-4文字、絵文字対応）とボタン色（16色パレット）を自由に設定
- **ワンクリック適用**: プリセットボタンをクリックして即座に検索条件を適用
- **視覚的フィードバック**: アクティブなプリセットは太枠で表示、検索条件を手動変更すると自動解除
- **簡単編集**: 右クリックでプリセットの編集・削除が可能
- **クリアボタン**: ×ボタンで検索文字列とタグフィルターを一括リセット
- **ワークスペース独立**: 各ワークスペースで独自のプリセットを管理
...
### ⚙️ 豊富なエディタ設定
- **タブ化された設定画面**: エディタ、タグ、Aboutの3つのタブで整理
- **エディタタブ**: キーバインド（Vim、Emacs、VSCodeなど）、テーマ1・2、フォントサイズ、行間（1.0-2.5）、折り返し、行番号表示
- **タグタブ**: タグの作成、編集、削除、ドラッグ&ドロップでの並べ替え
- **Aboutタブ**: GitHubリポジトリ、ダウンロードサイト、Buy Me A Coffeeへのリンク
- **空白文字表示**: スペース・タブの可視化
...
- **コンテキストメニュー**:
  - ファイル: ファイル名更新・削除
  - ステータスバー: 開発者ツールアクセス
  - エディタ: URL開く・Google検索・標準編集機能・箇条書き操作・表操作
- **箇条書き機能**: 選択行の一括操作
  ...
  - Undo/Redo完全対応
- **表編集機能**: マークダウン表のWYSIWYG編集
  - 表を追加: 新規3×3表をカーソル位置に挿入
  - 表を編集: 既存の表をモーダルダイアログで編集
  - 行・列の追加・削除、セル内改行対応（Enterキー）、列ごとの配置設定（左/中央/右）
  - プレビューで配置と改行が正しく表示
...
### ⌨️ 高度なキーボード操作
- **新規メモ作成**: `Cmd+N` (macOSのみ) で新しいメモを開く
- **タブ切り替え**: `Ctrl+Tab` で次のタブ、`Ctrl+Shift+Tab` で前のタブに切り替え
...
### 🔄 自動更新チェック
- **起動時チェック**: アプリケーション起動時に自動的に新バージョンを確認
- **定期チェック**: 24時間ごとにバックグラウンドでバージョンチェック
- **更新通知**: 新バージョンが利用可能な場合、設定ボタンの右に赤いアイコン（browser_updated）を表示
- **ワンクリックダウンロード**: 更新アイコンをクリックすると、ダウンロードサイトをブラウザで開く
- **セマンティックバージョン比較**: 現在のバージョンと最新バージョンを正確に比較
...
- **settings.json**: エディタ設定（キーバインド、テーマ、フォントサイズ、行間等）
...
#### バージョンチェック
- `check-update`: 新バージョンの確認（https://xpenguin.biz/memo3/からバージョン情報を取得）
